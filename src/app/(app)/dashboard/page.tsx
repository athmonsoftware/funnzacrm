    ...
    <main className="min-h-screen bg-[#f6f8fb] px-4 py-5 text-[#14213d] sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <section className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-semibold text-[#16a34a]">Workspace overview</p>
            <h1 className="mt-2 text-2xl font-semibold tracking-normal sm:text-3xl">
              AI operating system for business communication
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[#64748b]">
              Monitor customers, conversations, payments, and AI automation from one fast workspace.
            </p>
          </div>
            ...
            return (
              <Card key={metric.id} className="p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Badge tone={isDown ? "amber" : "green"}>{metric.change}</Badge>
                </div>
          ...
          <Card>
            ...
            <div className="grid gap-5 p-5 lg:grid-cols-2">
              <MiniBarChart
                ...
                data={customerGrowthData.map((item) => ({ label: item.month, value: item.customers }))}
                tone="green"
              />
              <MiniBarChart
                title="Revenue trends"
                data={revenueData.map((item) => ({ label: item.month, value: item.revenue }))}
                tone="blue"
              />
              <StackedMessageChart />
              <div className="rounded-md border border-[#edf1f5] p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-[#64748b]">Resolution mix across live conversations.</p>
                  </div>
                  <Badge tone="blue">78.5%</Badge>
                </div>
                <div className="mt-5 space-y-4">
                  <MetricProgress label="Resolved" value={aiPerformanceData.resolved} tone="bg-[#16a34a]" />
                  <MetricProgress label="Escalated" value={aiPerformanceData.escalated} tone="bg-[#4f46e5]" />
                  <MetricProgress label="Pending" value={aiPerformanceData.pending} tone="bg-[#f59e0b]" />
                </div>
              </div>
            </div>
          </Card>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="min-w-0">
                        ...
                        <span className="block truncate text-xs text-[#64748b]">{action.description}</span>
                      </span>
                    </button>
                  )
                })}
            ...
            <Card>
              ...
              <div className="space-y-4 p-5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold">{subscriptionStatus.plan}</span>
                </div>
                <UsageBar label="SMS usage" value={subscriptionStatus.smsUsed} limit={subscriptionStatus.smsLimit} />
                <UsageBar label="WhatsApp usage" value={subscriptionStatus.whatsappUsed} limit={subscriptionStatus.whatsappLimit} />
                <UsageBar label="AI usage" value={subscriptionStatus.aiTokensUsed} limit={subscriptionStatus.aiTokensLimit} compact />
                <p className="rounded-md bg-[#f8fafc] px-3 py-2 text-xs text-[#64748b]">
                  Next billing: <span className="font-semibold text-[#14213d]">{subscriptionStatus.nextBilling}</span>
                </p>
              </div>
            </Card>
          </div>
        ...
        <section className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
          <Card>
            <SectionHeader title="Recent conversations" action={<Button tone="secondary">Open inbox</Button>} />
            <div className="divide-y divide-[#edf1f5]">
              {conversations.slice(0, 5).map((conversation) => (
                <div key={conversation.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_auto] sm:items-center">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="font-semibold">{conversation.customer}</p>
                      {conversation.unread ? <span className="h-2 w-2 rounded-full bg-[#16a34a]" /> : null}
                    </div>
                    <p className="mt-1 line-clamp-1 text-sm text-[#64748b]">{conversation.preview}</p>
                  </div>
          ...
          <Card>
            <SectionHeader title="Live activity" description="Workspace events from customers, AI, payments, and teams." />
            <div className="divide-y divide-[#edf1f5]">
...
function StackedMessageChart() {
  const max = Math.max(...messageVolumeData.map((item) => item.sms + item.whatsapp + item.ai))
...
function MetricProgress({ label, value, tone }: { label: string; value: number; tone: string }) {
  return (
    <div>
