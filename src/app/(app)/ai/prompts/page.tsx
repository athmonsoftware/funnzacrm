            ...
            <Card key={prompt.id}>
              ...
              <div className="p-5">
                <div className="flex gap-3">
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-[#f1f5f9] text-[#14213d]">
                    {prompt.name.includes("Payment") ? <ShieldCheck size={18} /> : prompt.name.includes("Support") ? <MessageSquareText size={18} /> : <FileText size={18} />}
                  </span>
                  <div>
                    <p className="text-sm leading-6 text-[#64748b]">{prompt.description}</p>
                    <pre className="mt-4 max-h-52 overflow-auto whitespace-pre-wrap rounded-md border border-[#edf1f5] bg-[#fbfdff] p-4 font-sans text-sm leading-6 text-[#334155]">
                      {prompt.content}
                    </pre>
                  </div>
                </div>
